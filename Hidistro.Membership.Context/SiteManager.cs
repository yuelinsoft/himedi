...
namespace Hidistro.Membership.Context
{
    ...
    public class SiteManager : IUser
    {
        ...
        public bool ChangePassword(string newPassword)
        {
            if (HiContext.Current.User.UserRole == Hidistro.Membership.Core.Enums.UserRole.SiteManager)
            {
                ...
                if (((user != null) && (user.UserId != this.UserId)) && user.IsAdministrator)
                {
                    ...
                    return (this.MembershipUser.Membership.ChangePassword(oldPassword, newPassword) && this.ReEncryptKey(oldPassword, newPassword));
                }
            }
            return false;
        }
        ...
        {
            if (!this.MembershipUser.Membership.ChangePassword(password, newPassword))
            {
                return false;
            }
            return ((password == newPassword) || this.ReEncryptKey(password, newPassword));
        }
