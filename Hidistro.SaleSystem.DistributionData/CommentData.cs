namespace Hidistro.SaleSystem.DistributionData
{
    ...
    public class CommentData : CommentSubsiteProvider
    {
       Database database = DatabaseFactory.CreateDatabase();
        ...
        public override IList<ArticleInfo> GetArticleList(int? maxNum, string category, string keyWords)
        {
            StringBuilder builder = new StringBuilder();
            if (maxNum.HasValue)
            {
                builder.AppendFormat("SELECT TOP {0} * ", maxNum.Value);
            }
            else
            {
                builder.Append("SELECT * ");
            }
            builder.AppendFormat(" FROM vw_distro_Articles WHERE DistributorUserId = {0} and IsRelease=1", HiContext.Current.SiteSettings.UserId.Value);
            if (!string.IsNullOrEmpty(category))
            {
                builder.AppendFormat(" AND CategoryId IN {0}", category);
            }
            if (!string.IsNullOrEmpty(keyWords))
            {
                keyWords = DataHelper.CleanSearchString(keyWords);
                string[] strArray = Regex.Split(keyWords.Trim(), @"\s+");
                builder.AppendFormat(" AND (Title LIKE '%{0}%'", DataHelper.CleanSearchString(strArray[0]));
                for (int i = 1; (i < strArray.Length) && (i <= 5); i++)
                {
                    builder.AppendFormat(" OR Title LIKE '%{0}%'", DataHelper.CleanSearchString(strArray[i]));
                }
                builder.Append(")");
            }
        ...
        public override DataSet GetArticlProductList(int articlId)
        {
            return new DataSet();
        }
        ...
        public override DataTable GetPromotes(Pagination pagination, int promotiontype, out int totalPromotes)
        {
            ...
            {
                str2 = string.Format("SELECT TOP 10 * FROM distro_Promotions WHERE DistributorUserId={0} ", HiContext.Current.SiteSettings.UserId.Value);
            }
            else
            {
                str2 = string.Format("SELECT TOP {0} * FROM distro_Promotions WHERE  DistributorUserId={1}  AND ActivityId NOT IN (SELECT TOP {2} ActivityId FROM distro_Promotions WHERE AND DistributorUserId={3} )  ", new object[] { pagination.PageSize, HiContext.Current.SiteSettings.UserId.Value, pagination.PageSize * (pagination.PageIndex - 1), HiContext.Current.SiteSettings.UserId.Value });
            }
            if (promotiontype != 0)
            {
