<%@ Page Language="C#" EnableSessionState="True" AutoEventWireup="true" CodeFile="SubmitTaobaoPurchaseorderCart.aspx.cs" Inherits="Hidistro.UI.Web.Shopadmin.purchaseOrder.SubmitTaobaoPurchaseorderCart" %>
...
<title>易分销---采购单</title>
    ...
    <script type="text/javascript">
        function ShowAddDiv(productId)
        {
          ...
          $("#pages").text("");
          DivWindowOpen(600,500,'divWindow');         
        }
        ...
        function GetSearchData(pageindex) {
            $.getJSON(
                "ReturnSearchPurchaseProduct.aspx",
                { serachName: $("#serachName").val(), serachSKU: $("#serachSKU").val(), page: pageindex },
                function(json) {                
                    var currentPage = $("#currentPage").val();
                    var pageCount = Math.ceil(json.recCount / 8);
...
<tr>
  <td align="left"><strong><asp:CheckBox Checked="true" ID="chkTbOrder" runat="server" Text='淘宝订单号：' /></strong><span><%# DataBinder.Eval(Container.DataItem, "orderId")%></span></td>
  <td width="44%" align="right"><strong>下单时间：</strong><span><%# DataBinder.Eval(Container.DataItem, "createTime")%></span></td>
</tr>
</table>
                              ...
                              <tr>
                               ...
                               <tr>
                                 ...
                                 <td class="td1" colspan="5">
                                  <div class="Operating">
                                    <div class="wordInfo"><strong><%# DataBinder.Eval(Container.DataItem, "title")%></strong>&nbsp;<span>X <%# DataBinder.Eval(Container.DataItem, "number")%></span></div>
                                    <div class="submit">                                    
                                    <asp:Button ID="productDel" CommandArgument='<%# DataBinder.Eval(Container.DataItem, "id")%>' OnCommand="ButtonDelete_Command" runat="server"  Text="删除" CssClass="delete"/>
                                    </div>                                  
                                  </div>
                                 </td>
                               </tr> 
                               <tr>
                                 <td>
                                   ...
                                   <ul class="ul1"></ul>
                                   <ul class="ul2">
                                     <label runat="server" visible='<%#  ((DataBinder.Eval(Container.DataItem, "localSKU")).ToString())==""?true:false %>'>请点击查找选择商品</label>
                                     <%# DataBinder.Eval(Container.DataItem, "localSKU")%>
                                   </ul>
                                   </div> 
                                   </td>
            <td style="TEXT-ALIGN: left"><%# DataBinder.Eval(Container.DataItem, "localProductName")%><br />
           <div class="submit">
                   <input type="button" onclick='ShowAddDiv("<%# DataBinder.Eval(Container.DataItem, "id")%>",600,500)' value='<%#  ((DataBinder.Eval(Container.DataItem, "localSKU")).ToString())==""?"查找":"修改" %>' class="searchbutton"/>
                   <span class="save"><asp:CheckBox runat="server" Text="保存对应关系" Checked="true" ID="chkSave" Visible='<%#  ((DataBinder.Eval(Container.DataItem, "localSKU")).ToString())==""?false:true %>'/>
                   ...
                   <asp:Label style="display:none;" ID="lblSpec" runat="server" Text='<%# DataBinder.Eval(Container.DataItem, "spec")%>' /> 
                                 ...
                                 <td title="进货价"><label runat="server" visible='<%#  ((DataBinder.Eval(Container.DataItem, "localSKU")).ToString())!=""?true:false %>'>￥</label><%# DataBinder.Eval(Container.DataItem, "localPrice")%></td>
                                 <td>
                                 <div class="control" runat="server" visible='<%#  ((DataBinder.Eval(Container.DataItem, "localSKU")).ToString())!=""?true:false %>'> 
                                  <asp:TextBox CssClass="forminput" ID="productNumber" runat="server" ReadOnly="true" Text='<%# DataBinder.Eval(Container.DataItem, "number")%>'/>
                                  <a class="alink1"><asp:Button ID="productAdd" runat="server" Text="" CommandArgument='<%# DataBinder.Eval(Container.DataItem, "id")%>' OnCommand="ButtonAdd_Command"/></a>
                                  <a class="alink2"><asp:Button ID="productDec" runat="server" Text="" CommandArgument='<%# DataBinder.Eval(Container.DataItem, "id")%>' OnCommand="ButtonDec_Command"/></a>
                                 </div></td>
                                 <td class="td2" title="存货"><%# DataBinder.Eval(Container.DataItem, "localStock")%></td>                                 
                               </tr>
                            </tbody>
        </table>
 </ItemTemplate>
</asp:Repeater>  
<table class="tableShow1"><tr><th align="right">合计：<%# DataBinder.Eval(Container.DataItem, "orderCost")%></th></tr></table>
</div>
</ItemTemplate>
</asp:Repeater>
      ...
      <div class="dataarea mainwidth databody">  
        <div class="title title_height m_none td_bottom"> <em><img height="32" width="32" src="../images/05.gif"></em>
        <h1 class="title_line">注意：正确填写收货人地址，以便尽快收到商品！</h1>
      </div>        
        <div>
	      <div class="datafrom">
	      ...
	      <ul>
	        <li> 
	         <span class="formitemtitle Pw_198">收货人姓名：</span>
	          <asp:TextBox ID="txtShipTo" runat ="server" CssClass="forminput"></asp:TextBox>
	           <p runat="server" id="txtShipToTip">姓名长度在20个字符以内</p>
	        </li>
	        ...
	        <li> <span class="formitemtitle Pw_198">邮政编码：</span>
	         <asp:TextBox ID="txtZipcode" runat ="server" CssClass="forminput"></asp:TextBox>
	          <p runat="server" id="txtZipcodeTip">邮编长度限制在6-10个字符之间,只能输入数字</p>
            </li>
	        ...
	        <li><span class="formitemtitle Pw_198">配送方式：</span>
	           <span style="float:left;"><Hi:ShippingModeRadioButtonList ID="radioShippingMode" runat="server" RepeatDirection="Horizontal" RepeatColumns="4" /></span>            
	        </li>
	       <li class="clear">&nbsp;</li>
          </ul>
           <ul  class="btntf Pa_198">
             <asp:Button runat="server" ID="btnSubmit" OnClick="btnSubmit_Click" Text="提交采购单"  OnClientClick="return PageIsValid();" CssClass="submit_DAqueding inbnt"/>
          </ul>          
            </div>
          </div>           
        </div>  
...
<div class="Pop_up" id="divWindow" style="display:none;">
  <h1>订单---查找本地商品</h1>
    <div class="img_datala"><a onclick="CloseDiv('divWindow')" href="#"><img height="20" width="38" src="../images/icon_dalata.gif" style="cursor: pointer;"></a></div>
       ...
       <ul>
          <table width="100%">
                           <tbody><tr>
                            ...
                            <td nowrap="nowrap">商家编码：</td>
                            <td><input id="serachSKU"  class="forminput" /></td>
        ...
        <td><input type="hidden" name="serachProductId" id="serachProductId" />
         <asp:Button ID="btnMatch" runat="server" Text="确&nbsp;&nbsp;定" onclick="btnMatch_Click" CssClass="submit_DAqueding"  />
         </td>
  </tr>
</table>
</ul>
<ul style="padding-top:10px; clear: both;">
<input  type="hidden" name="currentPage" id="currentPage" /><span id="pages"></span><a id="prevLink" onclick="prevPage()" href="#">上一页</a> &nbsp;&nbsp;&nbsp;<a  id="nextLink" onclick="nextPage()" href="#">下一页</a>
</ul>
</div>
</div>
</form>
