<%@ Page Title="" Language="C#" MasterPageFile="~/Shopadmin/ShopAdmin.Master" AutoEventWireup="true" CodeFile="Default.aspx.cs" Inherits="Hidistro.UI.Web.Shopadmin.Default" %>
                    ...
                    <Columns>
                        ...
                        <asp:TemplateField HeaderText="采购单编号" HeaderStyle-CssClass="info">
                         ...
                         <asp:TemplateField HeaderText="采购单状态" HeaderStyle-CssClass="estate">
                            ...
                            <Hi:OrderRefundStatusMark runat="server" ID="OrderRefundStatusMark" NavigateUrl='<%# Globals.ApplicationPath + "/Shopadmin/purchaseOrder/RefundPurchaseDetails.aspx?PurchaseOrderId="+Eval("PurchaseOrderId" )%>' Status='<%# Eval("RefundStatus") %>' />
                        ...
                        <asp:TemplateField HeaderText="采购单操作" HeaderStyle-CssClass="handle">
                            <itemtemplate>
                              <Hi:DistributorPurchaseOrderDetailsHyperLink ID="lkbtnPurchaseOrderDetails" PurchaseStatusCode='<%# Eval("PurchaseStatus") %>' PurchaseOrderId ='<%# Eval("PurchaseOrderId") %>' Target="_blank" Text="查看详情" runat="server" ForeColor="BlueViolet"></Hi:DistributorPurchaseOrderDetailsHyperLink><br />
	                           <span class="submit_faihuo"><asp:HyperLink ID="lkbtnPay" runat="server" NavigateUrl='<%# Globals.ApplicationPath+ "/Shopadmin/purchaseOrder/Pay.aspx?PurchaseOrderId="+ Eval("PurchaseOrderId") %>'  Target="_blank" Text="付款" Visible="false"></asp:HyperLink></span>
    ...
    <ul class="up Pa_160">
      <asp:Button ID="btnClosePurchaseOrder"  runat="server" CssClass="submit_DAqueding" OnClientClick="return ValidationCloseReason()" Text="确 定"   />
				...
				<div class="orderdata" id="divOrders">
                    ...
                    <Columns>
                        ...
                        <asp:TemplateField HeaderText="订单状态" HeaderStyle-CssClass="estate" >
                          ...
                          <Hi:OrderRefundStatusMark runat="server" ID="hpkOrderRefund" NavigateUrl='<%#Globals.ApplicationPath+( "/Shopadmin/sales/RefundOrderDetails.aspx?OrderId="+ Eval("OrderId")) %>' Target="_blank" Status ='<%# Eval("RefundStatus") %>'></Hi:OrderRefundStatusMark>
                        ...
                        <asp:TemplateField HeaderStyle-CssClass="handle">
                            <itemtemplate>
                               <Hi:DistributorOrderDetailsHyperLink ID="lkbtnOrderDetails" OrderStatusCode='<%# Eval("OrderStatus") %>' OrderId='<%# Eval("OrderId") %>' Target="_blank" Text="详情" runat="server" ForeColor="BlueViolet"/><br />
			...
			<div class="instantstat clearfix" id="divSendPurchseOrders">
				注：这里显示最近的<asp:Label ID="lblPurchaseOrderNumbers" runat="server"></asp:Label>笔采购单，有“退”字代表该采购单退过款。<asp:HyperLink ID="allPurchaseOrder" runat="server"   Target="_blank" Text="查看生成的采购单"  ForeColor="Blue"></asp:HyperLink>&nbsp;&nbsp;<asp:HyperLink ID="allPurchaseOrder2" runat="server" Target="_blank" Text="查看提交的采购单"  ForeColor="Blue"></asp:HyperLink>
			</div>
		</div>
