<%@ Page Title="" Language="C#" MasterPageFile="~/Admin/Admin.Master" AutoEventWireup="true" CodeFile="BatchSendPurchaseOrderGoods.aspx.cs" Inherits="Hidistro.UI.Web.Admin.purchaseOrder.BatchSendPurchaseOrderGoods" %>
...
<div class="dataarea mainwidth databody">
    <div class="title"> <em><img src="../images/05.gif" width="32" height="32" /></em>
        <h1>分销商采购单批量发货</h1>
        <span>这里的订单将批量执行分销商采购单发货操作，请先选择要发货订单的配送方式跟快递单号，然后执行批量发货按钮即可</span></div>	
     ...
     <UI:Grid ID="grdOrderGoods" runat="server" ShowHeader="true" AutoGenerateColumns="false" DataKeyNames="PurchaseOrderId" HeaderStyle-CssClass="table_title" GridLines="None" Width="100%">
            <Columns>
                <asp:TemplateField HeaderText="采购单编号">
                    <itemtemplate>
                         <Hi:PurchaseOrderDetailsHyperLink ID="lkbtnPurchaseOrderDetails" PurchaseStatusCode='<%# Eval("PurchaseStatus") %>' PurchaseOrderId ='<%# Eval("PurchaseOrderId") %>' Target="_blank" Text='<%# Eval("PurchaseOrderId") %>' runat="server"/>
                    </itemtemplate>
                </asp:TemplateField>
