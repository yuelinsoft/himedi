<%@ Page Language="C#" AutoEventWireup="true" CodeFile="PrintPurchaseOrder.aspx.cs" Inherits="Hidistro.UI.Web.Admin.purchaseOrder.PrintPurchaseOrder" %>
...
<body>
    ...
    <div class="dataarea mainwidth" style="border-style:none;">
                ...
                <UI:Grid runat="server" ID="grdOrderItems" AutoGenerateColumns="false" Width="100%" BorderWidth="0">
                    <Columns>
                        ...
                        <asp:TemplateField HeaderText="货号">
                            <ItemTemplate>
                                <span class="colorC">货号：<%#Eval("sku") %></span>
	                                <span><%# Eval("SKUContent") %></span>
                            </ItemTemplate>
                        </asp:TemplateField>
                        <asp:BoundField DataField="ItemDescription" HeaderText="商品" />
                        <asp:BoundField DataField="ItemPurchasePrice" HeaderText="商品单价" />
                        <asp:BoundField DataField="Quantity" HeaderText="购买数量" />
                        <asp:BoundField DataField="Quantity" HeaderText="发货数量" />
                        <asp:TemplateField HeaderText="小计">
                            <ItemTemplate>
                                <%# (decimal)Eval("ItemPurchasePrice") * (int)Eval("Quantity")%>
                            </ItemTemplate>
                        </asp:TemplateField>
                    </Columns>
                </UI:Grid>
        ...
        <div class="Order_Info">
            <table cellpadding="3" cellspacing="0" width="100%">
                <tr>
                    <td>商品金额：<Hi:FormatedMoneyLabel ID="litItemsMoney" runat="server" /></td>
                    <td>运费：<Hi:FormatedMoneyLabel ID="litShippMoney" runat="server" /></td>
                    <td>支付手续费：<Hi:FormatedMoneyLabel ID="litPayMoney" runat="server" /></td>
                </tr>
                <tr>
                    <td>订单选项费用：<Hi:FormatedMoneyLabel ID="litOptionMoney" runat="server" /><asp:Literal ID="litOptionDes" runat="server" /></td>
                    <td>优惠券抵扣金额：<Hi:FormatedMoneyLabel ID="litCouponMoney" runat="server" /></td>
                    <td>买家折扣：<Hi:FormatedMoneyLabel ID="litDiscountMoney" runat="server" /></td>
                </tr>
                <tr>
                    <td>满额打折活动：<Hi:FormatedMoneyLabel ID="litActivity" runat="server" /></td>
                    <td>订单实付金额：<Hi:FormatedMoneyLabel ID="litTotalMoney" runat="server" /></td>
                    <td></td>
                </tr>
            </table>
        </div>
