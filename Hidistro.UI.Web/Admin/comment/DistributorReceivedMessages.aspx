<%@ Page Title="" Language="C#" MasterPageFile="~/Admin/Admin.Master" AutoEventWireup="true" CodeFile="DistributorReceivedMessages.aspx.cs" Inherits="Hidistro.UI.Web.Admin.DistributorReceivedMessages" %>
...
<!--选项卡-->
	<div class="optiongroup mainwidth">
	  <ul>
	    <li class="menucurrent"><a href="javascript:void(0);"><span>收信箱</span></a></li>
	    <li><a href="DistributorSendedMessages.aspx"><span>发件箱</span></a></li>
        <li class="optionend"><a href="SendMessageToDistributor.aspx"><span>发送站内信给分销商</span></a></li>
      </ul>      	
</div>
	<div class="dataarea mainwidth">
    <div class="toptitle"> <em><img src="../images/07.gif" width="32" height="32" /></em> <span class="title_height">你可以查看回复删除分销商发送给你的站内消息.</span> </div>
    <!--搜索-->
    ...
    <!--数据列表区域-->
    <div class="datalist">
      <UI:Grid ID="messagesList" runat="server" ShowHeader="true" AutoGenerateColumns="false" DataKeyNames="ReceiveMessageId" GridLines="None" Width="100%" HeaderStyle-CssClass="table_title">
            <Columns>                 
                <UI:CheckBoxColumn HeaderStyle-CssClass="table_title"/>
                 ...
                 <asp:TemplateField HeaderText="标题" HeaderStyle-CssClass="td_right td_left">
                  <itemstyle  CssClass="Name" />
                    <ItemTemplate >
                        <a href='<%# Globals.GetAdminAbsolutePath(string.Format("/comment/ReplyDistributorReceivedMessages.aspx?ReceiveMessageId={0}", Eval("ReceiveMessageId")))%>' class='<%# Eval("IsRead").ToString()=="False"? "cstrong":"" %>'>
                ...
                <asp:TemplateField HeaderText="发件人" HeaderStyle-CssClass="td_right td_left">
                 <itemstyle  CssClass="Name" />
                    <ItemTemplate>
                        <a href='<%# Globals.GetAdminAbsolutePath(string.Format("/distribution/ManageDistributor.aspx?UserName={0}",Eval("Addresser"))) %>'><span class='<%# Eval("IsRead").ToString()=="False"? "cstrong":"" %>'><asp:Literal ID="litAddresser" runat="server" Text='<%# Eval("Addresser")%>' /></span></a>
                    </ItemTemplate>
                </asp:TemplateField>
                
                <asp:TemplateField HeaderText="时间" HeaderStyle-CssClass="td_right td_left">
                    <ItemTemplate>
                        <span class='<%# Eval("IsRead").ToString()=="False"? "cstrong":"" %>'><Hi:FormatedTimeLabel ID="litDateTime" ShopTime="true" runat="server" Time='<%#Eval("PublishDate") %>' /></span>
                    </ItemTemplate>
                </asp:TemplateField>
