...
namespace Hidistro.AccountCenter.DistributionData
{
    public class BusinessData : TradeSubsiteProvider
    {
        ...
        {
            DbCommand sqlStringCommand = database.GetSqlStringCommand("UPDATE distro_CouponItems SET UserId = @UserId WHERE ClaimCode = @ClaimCode");
        ...
        public override bool ChangeMemberGrade(int userId, int gradId, int points)
        {
            ...
            database.AddInParameter(sqlStringCommand, "CreateUserId", DbType.Int32, HiContext.Current.SiteSettings.UserId.Value);
        ...
        {
            DbCommand sqlStringCommand = database.GetSqlStringCommand("UPDATE distro_Orders SET OrderStatus = @OrderStatus WHERE OrderId = @OrderId");
        ...
        public override bool ConfirmOrderFinish(OrderInfo order)
        {
            ...
            database.AddInParameter(sqlStringCommand, "DistributorUserId", DbType.Int32, HiContext.Current.SiteSettings.UserId.Value);
            return (database.ExecuteNonQuery(sqlStringCommand) > 0);
        }

        public override bool ExitCouponClaimCode(string claimCode)
        {
            DbCommand sqlStringCommand = database.GetSqlStringCommand("SELECT COUNT(ClaimCode) FROM distro_CouponItems WHERE ClaimCode = @ClaimCode AND UserId IS NULL");
            database.AddInParameter(sqlStringCommand, "ClaimCode", DbType.String, claimCode);
        ...
        {
            DbCommand sqlStringCommand = database.GetSqlStringCommand("SELECT * FROM distro_Coupons WHERE NeedPoint > 0 AND ClosingTime > @ClosingTime AND DistributorUserId = @DistributorUserId");
        ...
        {
            DbCommand sqlStringCommand = database.GetSqlStringCommand("SELECT SUM(Quantity) FROM distro_OrderItems WHERE OrderId IN (SELECT OrderId FROM distro_Orders WHERE GroupBuyId = @GroupBuyId AND OrderStatus <> 1 AND OrderStatus <> 4 AND DistributorUserId=@DistributorUserId)");
            database.AddInParameter(sqlStringCommand, "GroupBuyId", DbType.Int32, groupBuyId);
            database.AddInParameter(sqlStringCommand, "DistributorUserId", DbType.Int32, HiContext.Current.SiteSettings.UserId.Value);
        ...
        public override OrderInfo GetOrderInfo(string orderId)
        {
            ...
            query = (query + "SELECT * FROM distro_OrderOptions WHERE OrderId = @OrderId;") + "SELECT * FROM distro_OrderGifts WHERE OrderId = @OrderId;" + "SELECT * FROM distro_OrderItems WHERE OrderId = @OrderId;";
        ...
        {
            DbCommand sqlStringCommand = database.GetSqlStringCommand("SELECT * FROM distro_PaymentTypes WHERE ModeId = @ModeId;");
        ...
        public override IList<PaymentModeInfo> GetPaymentModes()
        {
            ...
            database.AddInParameter(sqlStringCommand, "DistributorUserId", DbType.Int32, HiContext.Current.SiteSettings.UserId);
            using (IDataReader reader = database.ExecuteReader(sqlStringCommand))
            {
                while (reader.Read())
                {
        ...
        {
            DbCommand sqlStringCommand = database.GetSqlStringCommand("SELECT c.*, ci.ClaimCode FROM distro_CouponItems ci INNER JOIN distro_Coupons c ON c.CouponId = ci.CouponId WHERE ci.UserId = @UserId AND c.ClosingTime > @ClosingTime");
            database.AddInParameter(sqlStringCommand, "UserId", DbType.Int32, userId);
            database.AddInParameter(sqlStringCommand, "ClosingTime", DbType.DateTime, DateTime.Now);
            using (IDataReader reader = database.ExecuteReader(sqlStringCommand))
            {
        ...
        public override bool UpdateUserAccount(decimal orderTotal, int totalPoint, int userId)
        {
            ...
            database.AddInParameter(sqlStringCommand, "ParentUserId", DbType.Int32, HiContext.Current.SiteSettings.UserId.Value);
        ...
        {
            StringBuilder builder = new StringBuilder();
            builder.AppendFormat("UPDATE distro_Orders SET OrderStatus = {0}, PayDate = '{1}', GatewayOrderId = @GatewayOrderId,OrderPoint=@OrderPoint WHERE OrderId = '{2}';", 2, DateTime.Now, order.OrderId);
