       ...
       void CreateRow(DataRow row, DataTable dtSkus, StringBuilder sb)
        {
            ...
            sb.AppendFormat("<td><input type=\"text\" id=\"tdPurchasePrice_{0}\" style=\"width:60px\" value=\"{1}\" />", str, decimal.Parse(row["PurchasePrice"].ToString()).ToString("F2")).AppendLine();
            ...
            if (!string.IsNullOrEmpty(str))
            {
                ...
                if ((skuDistributorPrices != null) && (skuDistributorPrices.Rows.Count > 0))
                {
                    ...
                    for (int i = 7; i < skuDistributorPrices.Columns.Count; i++)
                    {
                        ...
                        sb.AppendFormat("<th class=\"td_right td_left\" scope=\"col\" style=\"width:110px\">{0}</th>", columnName);
                    }
                    sb.AppendLine("</tr>");
