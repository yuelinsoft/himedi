...
namespace Hidistro.Subsites.Data
{
    public class UnderlingData : UnderlingProvider
    {
        ...
        static string BuildUnderlingStatisticsQuery(SaleStatisticsQuery query)
        {
            if (null == query)
            {
        ...
        {
            DbCommand storedProcCommand = database.GetStoredProcCommand("sub_UnderlingBalanceDrawRequest_Update");
        ...
        {
            DbCommand storedProcCommand = database.GetStoredProcCommand("sub_Member_Delete");
            database.AddInParameter(storedProcCommand, "UserId", DbType.Int32, userId);
        ...
        {
            DbCommand sqlStringCommand = database.GetSqlStringCommand("DELETE FROM distro_MemberGrades WHERE GradeId = @GradeId AND IsDefault = 0 AND CreateUserId=@CreateUserId AND NOT EXISTS(SELECT * FROM distro_Members WHERE GradeId = @GradeId AND ParentUserId=@ParentUserId)");
        ...
        {
            DbCommand sqlStringCommand = database.GetSqlStringCommand(string.Format("SELECT * FROM distro_MemberGrades WHERE CreateUserId = {0} ORDER BY GradeId DESC;", HiContext.Current.User.UserId));
        ...
        public override DataTable GetUnderlingStatistics(SaleStatisticsQuery query, out int total)
        {
            ...
            database.AddInParameter(storedProcCommand, "sqlPopulate", DbType.String, BuildUnderlingStatisticsQuery(query));
            ...
            total = (int)database.GetParameterValue(storedProcCommand, "TotalProductSales");
            return table;
        }
        ...
        public override IList<UserStatisticsForDate> GetUserIncrease(int? year, int? month, int? days)
        {
            ...
            DbCommand sqlStringCommand = database.GetSqlStringCommand("SELECT (SELECT COUNT(*) FROM vw_distro_Members WHERE ParentUserId = @ParentUserId AND CreateDate >=@StartDate AND  CreateDate<= @EndDate) AS UserIncrease;");
            database.AddInParameter(sqlStringCommand, "ParentUserId", DbType.Int32, HiContext.Current.User.UserId);
