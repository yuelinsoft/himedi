...
{
    public partial class SetCategoryTemplate : AdminPage
    {
        private void BindData()
        {
            DropdownColumn column = (DropdownColumn)grdCategries.Columns[1];
            column.DataSource = GetThemes();
            grdCategries.DataSource = CatalogHelper.GetMainCategories();
            grdCategries.DataBind();
        }
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            {
                string virtualPath = HiContext.Current.GetSkinPath() + "/categorythemes/" + dropThmes.SelectedValue;
        ...
        private void btnUpload_Click(object sender, EventArgs e)
        {
            if (fileThame.HasFile)
            {
                ...
                {
                    string virtualPath = HiContext.Current.GetSkinPath() + "/categorythemes/" + GetFilename(Path.GetFileName(fileThame.PostedFile.FileName), Path.GetExtension(fileThame.PostedFile.FileName));
        ...
        {
            grdCategries.RowCommand += new GridViewCommandEventHandler(grdCategries_RowCommand);
