...
{
    public partial class SendMessageToDistributor : AdminPage
    {
        int userId;
        ...
        private void btnRefer_Click(object sender, EventArgs e)
        {
            if (ValidateValues())
            {
                ...
                if (userId == 0)
                {
                    Page.Response.Redirect(Globals.GetAdminAbsolutePath("/comment/SendMessageSelectDistributor.aspx"));
                }
                else if (userId > 0)
                {
                    Page.Response.Redirect(Globals.GetAdminAbsolutePath(string.Format("/comment/SendMessageSelectDistributor.aspx?UserId={0}", userId)));
                }
            }
        }
