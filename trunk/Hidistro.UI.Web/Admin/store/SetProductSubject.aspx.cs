...
namespace Hidistro.UI.Web.Admin
{
    ...
    public partial class SetProductSubject : AdminPage
    {
        int subjectId;
        string themName;
        ...
        private void btnSaveSubject_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                entity.SelectSingleNode("SubjectImg").InnerText = imgLogo.ImageUrl;
        ...
        private XmlNode GetSubProject()
        {
            string filename = HttpContext.Current.Request.MapPath(Globals.ApplicationPath + string.Format("/Templates/master/{0}/ProductSubjects.xml", themName));
            XmlDocument document = new XmlDocument();
            document.Load(filename);
            return document.SelectSingleNode("root/Subject[SubjectId='" + subjectId + "']");
        }
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                hLinkSubjects.NavigateUrl = Globals.GetAdminAbsolutePath("/store/ProductSubject.aspx?ThemName=" + themName);
                if (!base.IsPostBack)
                {
                    XmlNode subProject = GetSubProject();
                    if (subProject == null)
                    {
                        base.GotoResourceNotFound();
                    }
        ...
        private string UploadSubjectImg(HttpPostedFile postedFile)
        {
            ...
            string virtualPath = Globals.ApplicationPath + string.Format("/Templates/master/{0}/productsubject/{1}", themName, ResourcesHelper.GenerateFilename(Path.GetExtension(postedFile.FileName)));
