    ...
    public class ProductBasePage : AdminPage
    {
            ...
            if (outsiteLinkImgs.Count > 0)
            {
                foreach (string str3 in outsiteLinkImgs)
                {
                    ...
                    try
                    {
                        ...
                        productDescrip = productDescrip.Replace(str3, "http://" + siteSettings.SiteUrl + ((HttpContext.Current.Request.Url.Port == 80) ? "" : (":" + HttpContext.Current.Request.Url.Port)) + Globals.ApplicationPath + str + str4 + str5);
                        continue;
                    }
                    catch
                    {
        ...
        private IList<string> GetOutsiteLinkImgs(string html)
        {
            ...
            for (int i = 0; i < matchs.Count; i++)
            {
                ...
                if (((item.ToLower(CultureInfo.InvariantCulture).IndexOf(siteSettings.SiteUrl.ToLower(CultureInfo.InvariantCulture)) == -1) && (string.IsNullOrEmpty(siteSettings.SiteUrl2) || (item.ToLower(CultureInfo.InvariantCulture).IndexOf(siteSettings.SiteUrl2.ToLower(CultureInfo.InvariantCulture)) == -1))) && ((item.ToLower(CultureInfo.InvariantCulture).IndexOf(masterSettings.SiteUrl.ToLower(CultureInfo.InvariantCulture)) == -1) && (string.IsNullOrEmpty(masterSettings.SiteUrl2) || (item.ToLower(CultureInfo.InvariantCulture).IndexOf(masterSettings.SiteUrl2.ToLower(CultureInfo.InvariantCulture)) == -1))))
                {
                    list.Add(item);
                }
            }
