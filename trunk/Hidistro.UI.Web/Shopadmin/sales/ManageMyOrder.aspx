<%@ Page Title="" Language="C#" MasterPageFile="~/Shopadmin/ShopAdmin.Master" AutoEventWireup="true" CodeFile="ManageMyOrder.aspx.cs" Inherits="Hidistro.UI.Web.Shopadmin.ManageMyOrder" %>
...
<div class="dataarea mainwidth">
      ...
      <%--<div class="functionHandleArea clearfix m_none">
            <span>批量操作：</span>
            <Hi:ImageLinkButton ID="btnBatchSendGoods" runat="server" Text="全选筛选结果批量发货" DeleteMsg="从现搜索结果中筛选出已付款未发货的订单，是否继续？" IsShow="false"/>
      </div>--%>
  ...
  <div class="blank8 clearfix"></div>
      ...
      <table width="0" border="0" cellspacing="0">
	   ...
	   <tr class="td_bg">
		      ...
		      <td>&nbsp;</td>
		      <td>&nbsp;</td>
		      <td align="right"><a href="javascript:RemarkOrder('<%#Eval("OrderId") %>','<%#Eval("OrderDate") %>','<%#Eval("OrderTotal") %>','<%#Eval("ManagerMark") %>','<%#Eval("ManagerRemark") %>')"><Hi:OrderRemarkImage runat="server" DataField="ManagerMark" ID="OrderRemarkImageLink" /></a></td>
	        </tr>
		    <tr>
		      <td>
		      <asp:HyperLink runat="server" Target="_blank" NavigateUrl='<%# string.Format("../Underling/UnderlingDetails.aspx?userId={0}", Eval("UserId"))%>' Enabled='<%# Eval("UserId").ToString()!="1100" %>'><%#Eval("UserName")%></asp:HyperLink>
		      ...
		      <td><Hi:FormatedMoneyLabel ID="lblOrderTotal" Money='<%#Eval("OrderTotal") %>' runat="server" />
		        <span class="Name"><asp:HyperLink ID="lkbtnEditPrice" runat="server" NavigateUrl='<%# "EditMyOrder.aspx?OrderId="+ Eval("OrderId") %>'  Target="_blank" Text="修改价格" Visible="false" ForeColor="Blue"></asp:HyperLink></span>
		      ...
		      <td><Hi:OrderStatusLabel ID="lblOrderStatus" OrderStatusCode='<%# Eval("OrderStatus") %>' runat="server" /> 
               <span class="Name"><Hi:DistributorOrderDetailsHyperLink ID="lkbtnOrderDetails" OrderStatusCode='<%# Eval("OrderStatus") %>' OrderId='<%# Eval("OrderId") %>' Target="_blank" Text="详情" runat="server" ForeColor="BlueViolet"/></span>
		      ...
		      <td>
		      <Hi:ImageLinkButton ID="lkbtnCreatePurchaseOrder" IsShow="true" runat="server" Visible="false" Text="生成采购单" CommandArgument='<%# Eval("OrderId") %>' CommandName="CREATE_PURCHASEORDER"  DeleteMsg="只有在团购成功以后，才能为对应的团购订单成生采购单，确定生成采购单吗？" ForeColor="Red" />
		        <span class="submit_faihuo">&nbsp;<asp:HyperLink ID="lkbtnSendGoods" runat="server" NavigateUrl='<%# "SendMyGoods.aspx?OrderId="+ Eval("OrderId") %>' Target="_blank" Text="发货" Visible="false"  ForeColor="Red"></asp:HyperLink></span>
