...
namespace Hidistro.AccountCenter.DistributionData
{
    public class ProfileData : PersonalSubsiteProvider
    {
        ...
        {
            DbCommand sqlStringCommand = database.GetSqlStringCommand("INSERT INTO distro_BalanceDrawRequest (UserId,UserName,DistributorUserId,RequestTime, Amount, AccountName, BankName, MerchantCode, Remark) VALUES (@UserId,@UserName, @DistributorUserId,@RequestTime, @Amount, @AccountName, @BankName, @MerchantCode, @Remark)");
        ...
        static string BuildUnderlingBalanceDetailsQuery(BalanceDetailQuery query)
        {
        ...
        {
            DbCommand sqlStringCommand = database.GetSqlStringCommand("SELECT * FROM distro_InpourRequest WHERE InpourId = @InpourId;");
        ...
        {
            DbCommand sqlStringCommand = database.GetSqlStringCommand("SELECT * FROM distro_MemberGrades WHERE GradeId = @GradeId");
        ...
        {
            DbCommand sqlStringCommand = database.GetSqlStringCommand("SELECT * FROM distro_MemberGrades WHERE CreateUserId = @CreateUserId");
        ...
        public override IList<ShippingAddressInfo> GetShippingAddress(int userId)
        {
            ...
            DbCommand sqlStringCommand = database.GetSqlStringCommand("SELECT * FROM distro_ShippingAddresses WHERE  UserID = @UserID");
        ...
        {
            DbCommand sqlStringCommand = database.GetSqlStringCommand("SELECT count(ShippingId) as Count FROM distro_ShippingAddresses WHERE  UserID = @UserID");
            database.AddInParameter(sqlStringCommand, "UserId", DbType.Int32, userId);
            int num = 0;
            using (IDataReader reader = database.ExecuteReader(sqlStringCommand))
            {
        ...
        public override void GetStatisticsNum(out int noPayOrderNum, out int noReadMessageNum, out int noReplyLeaveCommentNum)
        {
            ...
            builder.AppendFormat("SELECT COUNT(*) AS NoReadMessageNum FROM distro_ReceivedMessages WHERE Addressee = '{0}' AND IsRead=0 ;", HiContext.Current.User.Username);
            builder.AppendFormat("SELECT COUNT(*) AS NoReplyLeaveCommentNum FROM distro_ProductConsultations WHERE UserId = {0} AND ReplyUserId is not null AND DistributorUserId={1};", HiContext.Current.User.UserId, HiContext.Current.SiteSettings.UserId);
            DbCommand sqlStringCommand = database.GetSqlStringCommand(builder.ToString());
            using (IDataReader reader = database.ExecuteReader(sqlStringCommand))
            {
        ...
        public override ShippingAddressInfo GetUserShippingAddress(int shippingId)
        {
            ...
            DbCommand sqlStringCommand = database.GetSqlStringCommand("SELECT * FROM distro_ShippingAddresses WHERE ShippingId = @ShippingId");
        ...
        {
            DbCommand sqlStringCommand = database.GetSqlStringCommand("DELETE FROM distro_InpourRequest WHERE InpourId = @InpourId");
            database.AddInParameter(sqlStringCommand, "InpourId", DbType.String, inpourId);
