    ...
    public class CommentData : CommentMasterProvider
    {
       Database database = DatabaseFactory.CreateDatabase();
        ...
        public override IList<ArticleInfo> GetArticleList(int? maxNum, string category, string keyWords)
        {
            ...
            builder.Append(" FROM vw_Hishop_Articles WHERE IsRelease=1");
        ...
        public override DataTable GetPromotes(Pagination pagination, int promotionType, out int totalPromotes)
        {
            ...
            {
                str2 = "SELECT TOP 10 * FROM Hishop_Promotions WHERE 1=1 ";
            }
            else
            {
                str2 = string.Format("SELECT TOP {0} * FROM Hishop_Promotions WHERE 1=1 AND ActivityId NOT IN (SELECT TOP {1} ActivityId FROM Hishop_Promotions) ", pagination.PageSize, pagination.PageSize * (pagination.PageIndex - 1));
            }
            if (promotionType != 0)
            {
