        ...
        public override object[] ParseProductData(params object[] importParams)
        {
            ...
            using (CsvReader reader = new CsvReader(new StreamReader(Path.Combine(str, "products.csv"), Encoding.Default), true, '\t'))
            {
                ...
                while (reader.ReadNextRecord())
                {
                    ...
                    if (!string.IsNullOrEmpty(reader[30]))
                    {
                        string path = Path.Combine(str + @"\products", reader[30]);
                        if (File.Exists(path))
                        {
                            row["Description"] = File.ReadAllText(path, Encoding.GetEncoding("gb2312"));
                        }
                    }
                    string str3 = this.Substring(reader[0x19]);
                    if (!string.IsNullOrEmpty(str3))
                    {
                        str3 = str3.Substring(str3.LastIndexOf(@"\") + 1);
                        if (File.Exists(Path.Combine(str + @"\products", str3)))
                        {
                            File.Copy(Path.Combine(str + @"\products", str3), current.Request.MapPath("~/Storage/Album/" + str3), true);
                            row["ImageUrl1"] = "/Storage/Album/" + str3;
                        }
                    }
                    str3 = this.Substring(reader[0x1a]);
                    if (!string.IsNullOrEmpty(str3))
                    {
                        str3 = str3.Substring(str3.LastIndexOf(@"\") + 1);
                        if (File.Exists(Path.Combine(str + @"\products", str3)))
