namespace Hishop.Transfers.HishopExporters
{
    ...
    public class Yfx1_2_to_Hishop5_4_2 : ExportAdapter
    {
        ...
        public Yfx1_2_to_Hishop5_4_2(params object[] exportParams) : this()
        {
        ...
        private void WriteProducts(XmlWriter productWriter)
        {
            ...
            foreach (DataRow row in this._exportData.Tables["products"].Rows)
            {
                ...
                TransferHelper.WriteCDataElement(productWriter, "SKU", row["ProductCode"].ToString());
                ...
                productWriter.WriteElementString("Upselling", (row["SaleStatus"].ToString() == "1") ? "1" : "0");
                productWriter.WriteElementString("marketPrice", row["MarketPrice"].ToString());
                DataRow[] rowArray = this._exportData.Tables["skus"].Select("ProductId=" + row["ProductId"].ToString(), "SalePrice");
                DataRow row2 = rowArray[0];
                productWriter.WriteElementString("Weight", row2["Weight"].ToString());
                if (this._includeCostPrice)
                {
                    productWriter.WriteElementString("CostPrice", row2["CostPrice"].ToString());
                }
                productWriter.WriteElementString("SalePrice", row2["SalePrice"].ToString());
                ...
                foreach (DataRow row3 in rowArray)
                {
                    ...
                    productWriter.WriteElementString("Price", (((decimal) row3["SalePrice"]) - ((decimal) row2["SalePrice"])).ToString());
                    DataRow[] rowArray2 = this._exportData.Tables["skuItems"].Select("SkuId='" + row3["SkuId"].ToString() + "'");
                    productWriter.WriteStartElement("skuItems");
                    foreach (DataRow row4 in rowArray2)
                    {
                        DataRow[] rowArray3 = this._exportData.Tables["attributes"].Select("AttributeId=" + row4["AttributeId"].ToString());
                        DataRow[] rowArray4 = this._exportData.Tables["values"].Select("ValueId=" + row4["ValueId"].ToString());
                        ...
                        productWriter.WriteElementString("ValueStr", rowArray4[0]["ValueStr"].ToString());
                        productWriter.WriteEndElement();
                    }
                    productWriter.WriteEndElement();
                    productWriter.WriteEndElement();
                ...
                productWriter.WriteEndElement();
                DataRow[] rowArray5 = this._exportData.Tables["productAttributes"].Select("ProductId=" + row["ProductId"].ToString());
                ...
                foreach (DataRow row5 in rowArray5)
                {
                    ...
                    productWriter.WriteElementString("AttributeId", row5["AttributeId"].ToString());
                    DataRow[] rowArray6 = this._exportData.Tables["values"].Select("ValueId=" + row5["ValueId"].ToString());
                    TransferHelper.WriteCDataElement(productWriter, "ValueStr", rowArray6[0]["ValueStr"].ToString());
                    productWriter.WriteEndElement();
                }
                productWriter.WriteEndElement();
                productWriter.WriteStartElement("images");
                if (row["ImageUrl1"].ToString().Length > 0)
                {
                    TransferHelper.WriteImageElement(productWriter, "image", this._includeImages, row["ImageUrl1"].ToString(), this._productImagesDir);
                }
                if (row["ImageUrl2"].ToString().Length > 0)
                {
                    TransferHelper.WriteImageElement(productWriter, "image", this._includeImages, row["ImageUrl2"].ToString(), this._productImagesDir);
                }
                if (row["ImageUrl3"].ToString().Length > 0)
                {
                    TransferHelper.WriteImageElement(productWriter, "image", this._includeImages, row["ImageUrl3"].ToString(), this._productImagesDir);
                }
                if (row["ImageUrl4"].ToString().Length > 0)
                {
                    TransferHelper.WriteImageElement(productWriter, "image", this._includeImages, row["ImageUrl4"].ToString(), this._productImagesDir);
                }
                if (row["ImageUrl5"].ToString().Length > 0)
                {
                    TransferHelper.WriteImageElement(productWriter, "image", this._includeImages, row["ImageUrl5"].ToString(), this._productImagesDir);
                }
                productWriter.WriteEndElement();
                productWriter.WriteEndElement();
            }
