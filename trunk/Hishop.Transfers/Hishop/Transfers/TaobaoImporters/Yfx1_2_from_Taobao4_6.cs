        ...
        public override object[] ParseProductData(params object[] importParams)
        {
            ...
            using (CsvReader reader = new CsvReader(new StreamReader(Path.Combine(str, "products.csv"), Encoding.Default), true, '\t'))
            {
                ...
                while (reader.ReadNextRecord())
                {
                    ...
                    {
                        row["Description"] = this.Trim(reader[0x18].Replace("\"\"", "\"").Replace("alt=\"\"", "").Replace("alt=\"", "").Replace("alt=''", ""));
                    }
                    ...
                    if (!string.IsNullOrEmpty(str2))
                    {
                        if (str2.EndsWith(";"))
                        {
                            ...
                            {
                                string str3 = strArray[i].Substring(0, strArray[i].IndexOf(":")) + ".jpg";
                        ...
                        {
                            File.Copy(Path.Combine(str + @"\products", str2.Replace(".jpg", ".tbi")), current.Request.MapPath("~/Storage/master/product/images/" + str2), true);
                            row["ImageUrl1"] = "/Storage/master/product/images/" + str2;
                        }
                    }
