...
namespace Hidistro.UI.SaleSystem.CodeBehind
{
    ...
    {
       const string FormFormat = "<form id=\"openidform\" name=\"openidform\" action=\"{0}\" method=\"POST\">{1}</form>";
       const string InputFormat = "<input type=\"hidden\" id=\"{0}\" name=\"{0}\" value=\"{1}\">";
        ...
        {
            return string.Format(CultureInfo.InvariantCulture, "<form id=\"openidform\" name=\"openidform\" action=\"{0}\" method=\"POST\">{1}</form>", new object[] { action, content + "<input type=\"submit\" value=\"信任登录\" style=\"display:none;\">" });
        }
