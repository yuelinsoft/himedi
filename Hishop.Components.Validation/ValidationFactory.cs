namespace Hishop.Components.Validation
{
    ...
    {
       static IDictionary<ValidatorCacheKey, Validator> attributeAndDefaultConfigurationValidatorsCache = new Dictionary<ValidatorCacheKey, Validator>();
       static object attributeAndDefaultConfigurationValidatorsCacheLock = new object();
       static IDictionary<ValidatorCacheKey, Validator> attributeOnlyValidatorsCache = new Dictionary<ValidatorCacheKey, Validator>();
       static object attributeOnlyValidatorsCacheLock = new object();
       static IDictionary<ValidatorCacheKey, Validator> defaultConfigurationOnlyValidatorsCache = new Dictionary<ValidatorCacheKey, Validator>();
       static object defaultConfigurationOnlyValidatorsCacheLock = new object();
       ...
       static Validator<T> CreateValidator<T>(string ruleset, bool cacheValidator)
        {
       ...
       static Validator CreateValidator(Type targetType, string ruleset, bool cacheValidator)
        {
       ...
       static Validator InnerCreateValidatorFromAttributes(Type targetType, string ruleset)
        {
       ...
       static Validator<T> WrapAndInstrumentValidator<T>(Validator validator)
        {
            return new GenericValidatorWrapper<T>(validator);
        }

       static Validator WrapAndInstrumentValidator(Validator validator)
        {
            return new ValidatorWrapper(validator);
        }

        [StructLayout(LayoutKind.Sequential)]
       struct ValidatorCacheKey : IEquatable<ValidationFactory.ValidatorCacheKey>
        {
           Type sourceType;
           string ruleset;
           bool generic;
