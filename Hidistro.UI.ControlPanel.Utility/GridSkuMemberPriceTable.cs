namespace Hidistro.UI.ControlPanel.Utility
{
    ...
    {
       void CreateRow(DataRow row, DataTable dtSkus, StringBuilder sb)
        {
            ...
            sb.AppendFormat("<td><input type=\"text\" id=\"tdCostPrice_{0}\" style=\"width:60px\" value=\"{1}\" />", str, (row["CostPrice"] != DBNull.Value) ? decimal.Parse(row["CostPrice"].ToString()).ToString("F2") : "").AppendLine();
            sb.AppendFormat("<td><input type=\"text\" id=\"tdSalePrice_{0}\" style=\"width:60px\" value=\"{1}\" />", str, decimal.Parse(row["SalePrice"].ToString()).ToString("F2")).AppendLine();
            for (int i = 7; i < dtSkus.Columns.Count; i++)
            {
                ...
                sb.AppendFormat("<td><input type=\"text\" id=\"{0}_tdMemberPrice_{1}\" name=\"tdMemberPrice_{1}\" style=\"width:60px\" value=\"{2}\" />{3}</td>", new object[] { columnName.Substring(0, columnName.IndexOf("_")), str, str3, str4 }).AppendLine();
            }
            sb.AppendLine("</tr>");
        }
