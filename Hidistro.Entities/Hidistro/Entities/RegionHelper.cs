namespace Hidistro.Entities
{
    ...
    {
       static System.Xml.XmlNode FindNode(int id)
        {
       ...
       static Dictionary<int, string> GetChildList(string xpath)
        {
            Dictionary<int, string> dictionary = new Dictionary<int, string>();
       ...
       static XmlDocument GetRegionDocument()
        {
        ...
        public static int GetRegionId(string county, string city, string province)
        {
            ...
            if (node != null)
            {
                ...
                xpath = string.Format("//city[@name='{0}']", city);
                node = node.SelectSingleNode(xpath);
                if (node != null)
                {
                    num = int.Parse(node.Attributes["id"].Value);
                    xpath = string.Format("//county[@name='{0}']", county);
                    node = node.SelectSingleNode(xpath);
                    if (node != null)
                    {
                        num = int.Parse(node.Attributes["id"].Value);
        ...
        public static int GetRegionIdByName(string province, string city, string county)
        {
            int num = 0;
            string xpath = string.Format("//province[@name='{0}']", province);
            System.Xml.XmlNode node = GetRegionDocument().SelectSingleNode(xpath);
            ...
            {
                xpath = string.Format("//city[@name='{0}']", city);
                System.Xml.XmlNode node2 = node.SelectSingleNode(xpath);
                if (node2 == null)
                {
                    return num;
                }
                xpath = string.Format("//county[@name='{0}']", county);
                System.Xml.XmlNode node3 = node2.SelectSingleNode(xpath);
                if (node3 != null)
                {
                    num = Convert.ToInt32(node3.Attributes["id"].Value);
                }
            }
            return num;
        }
