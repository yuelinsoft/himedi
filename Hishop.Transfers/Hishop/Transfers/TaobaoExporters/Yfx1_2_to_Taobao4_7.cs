namespace Hishop.Transfers.TaobaoExporters
{
    ...
    public class Yfx1_2_to_Taobao4_7 : ExportAdapter
    {
        ...
        public Yfx1_2_to_Taobao4_7(params object[] exportParams) : this()
        {
            ...
            foreach (DataRow row in this._exportData.Tables["products"].Rows)
            {
                ...
                if (row["ImageUrl1"] != DBNull.Value)
                {
                    ...
                    if (File.Exists(path))
                    {
                        ...
                        if ((str7.EndsWith(".jpg") || str7.EndsWith(".gif")) || ((str7.EndsWith(".jpeg") || str7.EndsWith(".png")) || str7.EndsWith(".bmp")))
                        {
                            str7 = str7.Replace(info.Extension.ToLower(), ".tbi");
                ...
                builder.AppendFormat(format, new object[] { 
                    Trim(Convert.ToString(row["ProductName"])), "0", "", "0", "", "", "b", rowArray[0]["SalePrice"], "0", num, "14", "0", "0", "0", "0", "", 
