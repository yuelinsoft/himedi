namespace Transfers.TaobaoImporters
{
    ...
    public class Hishop5_4_2_from_taobao4_6 : ImportAdapter
    {
        ...
        private readonly Target _source = new TbTarget("4.6");
        ...
        public override object[] ParseProductData(params object[] importParams)
        {
            ...
            using (CsvReader reader = new CsvReader(new StreamReader(Path.Combine(str, "products.csv"), Encoding.Default), true, '\t'))
            {
                ...
                while (reader.ReadNextRecord())
                {
                    ...
                    if (!string.IsNullOrEmpty(reader[0x18]))
                    {
                        row["Description"] = this.Trim(reader[0x18].Replace("\"\"", "\""));
                    }
                    string str2 = this.Trim(reader[0x23]);
                    if (string.IsNullOrEmpty(str2))
                    {
                        str2 = this.Trim(reader[0x19]);
                    }
                    if (!string.IsNullOrEmpty(str2))
                    {
                        if (str2.EndsWith(";"))
                        {
                            ...
                            {
                                string str3 = strArray[i].Replace(string.Format(":0:{0}:|", i), ".jpg");
                                if (str3.Contains("http:"))
                                {
                                    str3 = str3.Substring(0, str3.IndexOf("http:"));
                                }
                                if (File.Exists(Path.Combine(str + @"\products", str3.Replace(".jpg", ".tbi"))))
                                {
                                    File.Copy(Path.Combine(str + @"\products", str3.Replace(".jpg", ".tbi")), current.Request.MapPath("~/Storage/Album/" + str3), true);
                                    switch (i)
                                    {
                                        case 0:
                                            row["ImageUrl1"] = "/Storage/Album/" + str3;
                                            break;

                                        case 1:
                                            row["ImageUrl2"] = "/Storage/Album/" + str3;
                                            break;

                                        case 2:
                                            row["ImageUrl3"] = "/Storage/Album/" + str3;
                                            break;

                                        case 3:
                                            row["ImageUrl4"] = "/Storage/Album/" + str3;
                                            break;

                                        case 4:
                                            row["ImageUrl5"] = "/Storage/Album/" + str3;
                                            break;
                                    }
                                }
                            }
